...
```java
String content = "120kjadbfahp2	3rd0au09dfqew,l1d2k3nrahdf ru3ur'a;;asfh0ehf";
String reg = "[a-zA-Z]+";
...
/*
find 方法的作用
1. 根据指定规则，定位满足条件的子字符串
2. 找到后，将子字符串开始索引和结束索引 + 1 记录到 matcher 对象的 int[] groups 中。若存在分组（即表达式中存在小括号），则会将每组的索引也记录到 groups 数组中
3. 并且将 oldLast 属性记录为结束索引 + 1，即 find 方法下次执行开始位置

group 方法根据记录的位置截取子字符串
*/
while (m.find()) {
...
使用正则表达式去检索某些特殊字符的时候，需要用到转义符号，否则检索不到结果，甚至会报错的。需要用到转义符的字符包含 `.*-+()$/\?[]^{}`
...
| \\s  | 匹配空白字符（空格、制表符等）                              |              |                                                             |                        |
| \\S  | 匹配非空白字符                                              |              |                                                             |                        |
| .    | 匹配除 `\n` 以外的任意字符                                  | a..b         | 以 a 开头，b 结尾，中间包括 2 个任意字符的长度为 4 的字符串 | aaab、aefb、a35b、a#*b |
| \\d  | 匹配单个数字字符，相当于 `[0-9]`                            | \\d{3}(\\d)? | 包含 3 个或 4 个数字的字符串                                | 123、9876              |
| \\D  | 匹配单个非数字字符，相当于 `[^0-9]`                         | \\D(\\d)*    | 以单个非数字字符开头，后接任意个数字字符串                  | a、A342                |
| \\w  | 匹配单个数字、大小写字母和下划线字符，相当于 `[0-9a-zA-Z_]` | \\d{3}\\w{4} | 以 3 个数字字符开头的长度为 7 的数字字母字符串              | 234abcd、12345Pe       |
| \\W  | 匹配单个非数字、大小写字母字符，相当于`[^0-9a-zA-Z_]`       | \\W+\\d{2}   | 以至少 1 个非数字字母字符开头，2 个数字字符结尾的字符串     | #29、#?@10             |

在正则表达式中使用 `(?!)` 或 创建对象时使用`Pattern.compile(reg, Pattern.CASE_INSENSITIVE)` 表示不区分大小写
...
| 符号  | 含义                                | 示例        | 说明                                        | 匹配输入                |
| ----- | ----------------------------------- | ----------- | ------------------------------------------- | ----------------------- |
| `*`   | 指定字符重复 0 次或 n 次（无要求）  | `(abc)*`    | 仅包含任意个abc 的字符串，等效于 `\w*`      | abc、abcabcabc          |
| `+`   | 指定字符重复 1次或 n 次（至少一次） | `m+(abc)*`  | 以至少 1 个 m 开头，后接任意个 abc 的字符串 | m、mabc、mabcabc        |
| `?`   | 指定字符重复 0 或 1 次（最多一次）  | `m+abc?`    | 以至少 1 个 m 开头，后接 ab 或 abc 的字符串 | mab、mabc、mmmab、mmabc |
| `{n}` | 只能输入 n 个字符                   | `[abcd]{3}` | 由 abcd 中字符组成的任意长度为 3 的字符串   | abc、dbc、adc           |
| `{n,}`   | 指定至少 n 个匹配                          | `[abcd]{3,}`     | 由 abcd 中字符组成的任意长度不小于 3 的字符串                            | aab、dbc、aaabdc                         |
| `{n,m}`  | 指定至少 n 个但不多于 m 个匹配            | `[abcd]{3,5}`    | 由 abcd 中字符组成的任意长度不小于 3、不大于 5 的字符串                  | abc、abcd、aaaaa、bcdab                  |

Java 采用贪婪匹配，即`a{3,4}` 优先匹配较长子字符串 `aaaa`。如果希望使用非贪婪匹配，则需要在限定符后添加 `?` ，例如 `a{3,4}?` 优先匹配较短子字符串 `aaa`
...
| `\b` | 匹配目标字符串的边界   | `han\b`            | 这里说的字符串的边界指的是子串间有空格，或者是目标字符串的结束位置 | hanshunping、sp**han**、nn**han** |
| `\B` | 匹配目标字符串的非边界 | `han\B`            | 和 `\b` 的含义刚刚相反                                       | **han**shunping、sphan、nnhan     |
